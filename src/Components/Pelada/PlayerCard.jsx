...
export default function PlayerCard({ player, onRemove, variant = 'default', index = 0 }) {
  ...
  return (
    <motion.div
      ...
      className={`w-full flex items-center justify-between p-2.5 sm:p-3 rounded-xl border ${variants[variant]} backdrop-blur-sm`}
    >
      <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
        <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center ${iconVariants[variant]}`}>
      ...
      {onRemove && (
        <Button
          ...
          size="icon"
          onClick={onRemove}
          className="h-8 w-8 rounded-full hover:bg-red-100 hover:text-red-600 transition-colors shrink-0"
        >
